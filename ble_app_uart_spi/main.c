...
#define LED_STATUS  6//chan 6 nay la chan ung voi mach nho mau xanh?
#define TYPE_DATA   '0'
...
char status_on[] =        "was_on\r\n";
char status_off[] =       "was_off\r\n";
char status_really_on[] = "really_on\r\n";
char status_really_off[] ="really_off\r\n";
...
void pin_init(void)
{
  ...
  //nrf_gpio_cfg_output(LED_INVERT_EACH_TIME);

}
/* SEND FROM SLAVE --> MASTER: message[] = [id:'1'/'2'/....]_[type:'0'(data)/'1'(status)]_[content]  */
static void update_status(char status[])
{ 
  uint16_t _length = 0;
  ret_code_t err_code;
  char buff_status[30] = "";
  ...
  do
   {
      _length = (uint16_t)strlen(buff_status);//cho nay la no thuc hien ham xu ly chuoi 
      err_code = ble_nus_data_send(&m_nus,buff_status, &_length, m_conn_handle);//day la ham gui tu Peripheral (server) ve phia central(client)
...
/* SEND FROM SLAVE --> MASTER: message[] = [id:'1'/'2'/....]_[type:'0'(data)/'1'(status)]_[content]  */
static void update_data(char data[])
{ 
  uint16_t _length = 0;
  ret_code_t err_code;
  char buff_data[30] = "";
  ...
  do
   {
      _length = (uint16_t)strlen(buff_data);//cho nay la no thuc hien ham xu ly chuoi 
      err_code = ble_nus_data_send(&m_nus,buff_data, &_length, m_conn_handle);//day la ham gui tu Peripheral (server) ve phia central(client)
      if ((err_code != NRF_ERROR_INVALID_STATE) &&
          (err_code != NRF_ERROR_RESOURCES) &&
          (err_code != NRF_ERROR_NOT_FOUND))
      {
...
void send_string(char text[])
{
  uint16_t _length = 0;
  ret_code_t err_code;
  NRF_LOG_INFO("send_string: %s",text);
  do
   {
      _length = (uint16_t)strlen(text);//cho nay la no thuc hien ham xu ly chuoi 
      err_code = ble_nus_data_send(&m_nus,text, &_length, m_conn_handle);//day la ham gui tu Peripheral (server) ve phia central(client)
      if ((err_code != NRF_ERROR_INVALID_STATE) &&
          (err_code != NRF_ERROR_RESOURCES) &&
          (err_code != NRF_ERROR_NOT_FOUND))
      {
...
void light_on(void)
{
  if(flag_status == ON)
  {
    update_status(status_really_on);
  }
  else
  {
...
void light_off(void)
{
  if(flag_status == OFF)
  {
    update_status(status_really_off);
  }
  else
  {
...
void handle_message(char mess[])
{
  if((mess[0] == ID) || (mess[0] == ALL_ID))
  {
    ...
    {
      if(!strncmp(&mess[1],command_off,strlen(command_off)))
      {
        light_off();
      }
      else
      {
        update_status("InvalidCmd\r\n");
      }
    }
  }
  else
  {
    NRF_LOG_INFO("not me - ID:%c",ID);
  }
}
...
//ham nay la ham nhan chuoi ki tu tu phia central (client) roi gui no len may tinh thong qua UART bang Hercules
  if (p_evt->type == BLE_NUS_EVT_RX_DATA)
  {
    ...
    //buffer[p_evt->params.rx_data.length] = '\0';
    NRF_LOG_HEXDUMP_DEBUG(buffer,(uint16_t)strlen(buffer));
    handle_message(buffer);
    for(uint16_t count = 0; count < 20 ; count ++)//day la buoc xoa mang
    {
      buffer[count] = '\0';
    }
  }
...
//---------------------------------------------------------ADE--------------------------
    ...
    for (;;)
    {
      ...
      sprintf(fake_data,"U:"NRF_LOG_FLOAT_MARKER"_I:"NRF_LOG_FLOAT_MARKER"_X\r\n",NRF_LOG_FLOAT(pRMSRegs.AVValue),NRF_LOG_FLOAT(pRMSRegs.AIValue));
      //NRF_LOG_INFO("fake_data""abc");
